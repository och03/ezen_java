/*
	...
	3. 사용 목적
		...
		2. 인수[매개변수]/반환[리턴] : 인수에 따른 서로 다른 반환값 얻을 수 있다.
			- 인수[매개변수] 함수 안으로 들어가는 수 / 코드 [*생략가능]
 			- 반환[리턴] : 함수 밖으로 나오는 수 / 코드	[*생략가능]
 				...
 				-> 자판기
 						...
 						반환 : 선택한 음료수 [* 돈 과 선택에 따른 서로 다른 음료수]
 		...
 		3. 메모리 효율성 높일 수 있다.
 			- 함수 { } 안에서 '{'시작되고'}' 끝나면 메모리/코드 다 사라짐/초기화 -> 그래서 리턴으로 돌림
 		...
 		4. 함수 선언
 			...
 			2. 함수명 : 서로 다른 함수간의 식별용[ *js내 동일한 이름 불가
 				/ 키워드(let, const, for) 불가
 				
 			3. ( ) : 인수 [함수 안으로 들어가는 코드/변수/배열/객체 등] ,로 구분
 			
 			4. { } : '{'함수 시작 / '{'함수 끝
 			
 			5. return : 함수 종료/리턴 [*해당 함수를 호출했던 곳으로 반환/리턴]
 						return 3; 함수를 호출했던 곳으로 '3'반환
 						return;   반환값이 없으므로 단순 함수 종료
 						
 				ex) 유재석 이 핸드폰 이 고장나서 서비스센터 간다.
 		...
 		5. 함수 호출/사용
 				1. js : 함수명(인수1, 인수2, 인수3) 인수 없으면 안 넣어도 됨
...
function 함수4( x,y,z ){return 3;}	// 4. 인수 있다 / 반환값 없다. -> 함수종료 역할
function 함수5(){return 5;}			// 5. 인수 없다 / 반환값 있다. -> 함수종료 역할
function 함수6(){return;}				// 6. 인수 없다 / 반환값 = 단순 함수종료 역할
...
let 함수결과1 = 함수명4(3, 5, 20)	//7. 함수 종료 후 리턴/반환 값을 변수에 저장
let 함수결과2 = 함수명5()			//8. 함수 종료 후 리턴/반환 값을 변수에 저장
함수명6()							//9. 함수 종료 후 리턴/반환 값이 없으므로 별도로 저장 안함

//3. 함수 호출2
	//1. HTML 이벤트 함수 호출
	//	<input type="button" oneclick="함수명()">
	//	<button type="button" oneclick="함수명()">
	//2. 함수내에서 다른 함수 호출
function 함수7(){console.log('함수안에서 함수 호출 가능');}
function 함수8(){함수7();}
