...
//1. 배열 선언 : '학생리스트' 배열명으로 비어있는 배열 선언
let 학생리스트 = [] //초기에는 배열에 아무것도 없다. (1번 실행)
...
function 추가함수(){ // f start (클릭 할때마다 실행)
	alert('함수 실행했네....')
	...
	let snameInput = document.querySelector(".sname")
		...
		// Selector : 선택자 (id, class, 마크업 등)
			// 1. document.querySelector(".sname") : same 이라는 class명 가진 input 객체 호출
			// 2. 호출된 input객체를 'sname' 변수에 담았다.
			
	//2. 입력받은 데이터 가져오기
	let name = snameInput.value
			// 3. 변수명.value : 입력된 값 호출 (css도 가져올 수 있음)
			
	//3. 입력받은 데이터를 배열에 추가
	//* 유호성검사 : 배열에 이미 존재하면 push 안함, 존재하지 않으면 push
	학생리스트.indexOf(name) != -1 ? alert('이미 존재하는 학생명입니다[추가불가]') : 학생리스트.push(name)
			// 4. 배열명.push(데이터) : 해당 데이터를 배열에 추가
			
	//4. 배열 출력
			// document.write(학생리스트) write를 쓰면 덮어쓰기 됨 // 다 사라짐
			// 1. ul 객체 호출
...
// 3. 삭제 함수    (인수    )반환return
function 삭제함수(){   // 삭제할 데이터?? 입력받은 데이터 삭제
	//1. <input> 객체 호출
	let sanmeInput = document.querySelector('.sname')
	//2. <input> 객체의 value 속성 호출
	let name = sanmeInput.value
		// 삭제하려면 인덱스 필요!!?? 인덱스 몰라 --> 인덱스 찾자
	// 3. 삭제할 데이터의 인덱스(저장된 순번) 찾자
	let index = 학생리스트.lastIndexOf( name )
