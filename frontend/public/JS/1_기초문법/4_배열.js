...
배열 : 데이터 여러개를 한곳에 저장할 수 있는 공간/메모리/상자
	...
	7. 배열내 데이터 추가
		배열명.push(새로운값) : 배열내 마지막 위치에 저장
			-마지막 인덱스 뒤에 데이터 추가
		배열명.splice(위치인덱스, 0, 새로운 값) : 해당 위치인덱스에 새로운 값 저장
			- 특정 위치/인덱스 에 데이터 추가
			[*추가된 요소 뒤로 한칸씩 자동 밀려남]
	
	8. 배열내 데이터 삭제
		배열명.pop()	: 배열내 마지막 위치 데이터 삭제
		배열명.splice(제거할인덱스, 제거개수, [새로운값])
		ex)
	...
	9. 배열내 데이터를 이용한 데이터 삭제
		- 배열명.indexOf('찾을데이터') : 찾을데이터가 존재하면 찾은 인덱스 아니면 -1
		ex) 배열명.indexOf(배열명.indexOf('바나나'), 1) : '바나나'가 배열에 존재하면 삭제
...
document.write(배열명)
// 예제3 : 인덱스 사용
document.write( '<h3> 과일개수 : '+ 배열명.length + '</h3') // HTML 문자처리
...
document.write( 	'<li>'+배열명[4]+'</li>' ) // 변수or배열 문자처리x
document.write( '</ol>') // HTML 문자처리
...
console.log(배열명)
배열명.splice(0,0,'복숭아') // 0번째 인덱스에서 0개 자르고 '복숭아' 추가 (0, 1 넣으면 수박 사라짐)
console.log(배열명)

// 예제6 : 배열에 특정 요소 / 데이터 삭제
배열명.splice(0,1); // 0번부터 인덱스 1개 삭제
console.log(배열명) // '수박' 제거

배열명.splice() // '딸기' 삭제 하는데 인덱스 번호를 모른상태
	//1. 배열명. indexOf('딸기')=> 2
